...
def main():
    ...
    train_loader, val_loader = load.afew_faces_baseline(root_train, list_train, batchsize_train, root_eval, list_eval,
                                                        batchsize_eval)
...
def train(train_loader, model, optimizer, epoch, logger):
    ...
    for i, (input_var, target_var, index) in enumerate(train_loader):
        ...
        if i % 200 == 0:
            logger.print('Epoch: [{:3d}][{:3d}/{:3d}]\t'
                         ...
                         'Acc_Iter@1 {topframe.val:.3f} ({topframe.avg:.3f})\t'.format(
    ...
    logger.print(' *Acc@Video_soft {topsoft.avg:.3f}   *Acc@Frame {topframe.avg:.3f} '.format(topsoft=topVideoSoft,
                                                                                              topframe=topframe))
